...
class Iptables:
    ...
    def append_rule(self, chain: str, rule_spec: str) -> None:
        self.run(f"-A {chain} {rule_spec}")
        self._rules.append(IptablesRule(chain, rule_spec))

    def delete_rules(self) -> None:
        for rule in self._rules:
            self.run(f"-D {rule.chain} {rule.spec}")
        self._rules.clear()

    def run(self, opts: str) -> None:
        # run(f"{iptables_path} {opts} --bpf")
        run(f"{iptables_path} {opts}")
...
class BpfilterEndToEndTest(unittest.TestCase):
    ...
    def send(self, pkt: Any, iface: Device) -> Any:
        """Send a packet.
